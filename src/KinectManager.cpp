...
void KinectManager::guiEvent(ofxUIEventArgs &e) {
    ...
    if(e.widget->getName() == "KINECT_CAMERA_ANGLE") {
        ...
        kinect.setCameraTiltAngle( floor(slider->getScaledValue()) );
    }
...
ofColor KinectManager::getColorAt( int x , int y ) 
{
	if ( x > 0 && x < kinect.getWidth() ) 
	{
		if ( y > 0 && y < kinect.getHeight() ) 
		{
			ofColor c = kinect.getColorAt( x , y ) ; 
			if ( c.getBrightness() < minimumPixBrightness ) 
			{
				c.setBrightness( minimumPixBrightness ) ; 
			}
			return c ; 
		}
	}
