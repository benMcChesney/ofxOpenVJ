...
//--------------------------------------------------------------
KinectVertexShader_other::KinectVertexShader_other() {
...
//--------------------------------------------------------------
void KinectVertexShader_other::setup() {
...
//--------------------------------------------------------------
void KinectVertexShader_other::setupGui(float a_x, float a_y) {
   ...
   // gui = new ofxUICanvas(0, 0, length+xInit, ofGetHeight());
     ...
     */
    //
    ofAddListener( gui->newGUIEvent, this, &KinectVertexShader_other::guiEvent );
}

//--------------------------------------------------------------
void KinectVertexShader_other::guiEvent(ofxUIEventArgs &e) {
...
//--------------------------------------------------------------
void KinectVertexShader_other::activate() {
    
}

//--------------------------------------------------------------
void KinectVertexShader_other::deactivate() {
...
//--------------------------------------------------------------
void KinectVertexShader_other::update() {
    kinectMan->update( );
}

//--------------------------------------------------------------
void KinectVertexShader_other::draw() {
...
void KinectVertexShader_other::drawPointCloud( )
{
    int w = 640;
	int h = 480;
    /*
		...
		for(int x = 0; x < w; x += step) {
			if(kinectMan->kinect.getDistanceAt(x, y) > 0) {
                ...
                if ( vertex.z > kinectMan->pointCloudMinZ && vertex.z < kinectMan->pointCloudMaxZ )
                {
                    ...
                    ofPushMatrix() ;
                    ofQuaternion rot ;
                    ofQuaternion rotX = ofQuaternion( ofGetElapsedTimef() * 6.0f , ofVec3f( 0.0f , 1.0f , -0.25f  )) ; 
                    ofQuaternion rotY = ofQuaternion( normalizedZ , ofVec3f( 0.0f , 0.0f , 1.0f ) ) ;
                    rot = rotX * rotY ;
                    ofVec3f axis ;
                    float angle ;
                    rot.getRotate( angle , axis ) ;
                    
                    vertex.z += ofNoise( vertex.z + ofGetElapsedTimef() ) * 175.0f ;
                    //vertex.z += kinectMan->pointCloudZOffset ;
                    ofTranslate( vertex ) ;
                    ofRotate( angle , axis.x , axis.y , axis.z ) ;
                    ofScale( 1.0 , (sin ( ofGetElapsedTimef() ) + 3.25f ) * ( low + 0.5f ) , ( low + 0.01 ) * 50.0f ) ;
